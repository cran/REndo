...
#'through heteroscedastic covariance restrictions within the triangular system as proposed in Lewbel(2012). The function \code{heterogenIV}
...
#' allowing the user to include tradtional instrumental variables. If both robust=TRUE and !is.null(clustervar)
#' the function overrides the robust command and computes clustered standard errors and test statistics adjusted to
#' account for clustering. This function computes partial F-statistics that indicate potentially weak identification.
...
#Return Value
#'@return Returns an object of class \code{heterogenREndo}, with the following components:
...
#'summary(hIV)
#' @export
heterogenIV <- function(formula, data, clustervar = NULL, robust = TRUE){
    
    object <- new("heterogenREndo")
